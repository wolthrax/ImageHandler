...
public class MaxFilter extends AbstractFilter implements IFilter {

    private int matrixType;

    public MaxFilter(Matrix matrix) {
        super(matrix);
    ...
    public void handle() {
        ...
        for (int y = ConfigParams.Position.START + indent; y < height - indent; y++) {
            for (int x = ConfigParams.Position.START + indent; x < width - indent; x++) {
                ...
                Color color = new Color(maxPixelData.getR()/255d, maxPixelData.getG()/255d, maxPixelData.getB()/255d, 1);
